...
test('maps SQL-era filters and assignments to MongoDB values', () => {
  ...
  assert.deepEqual(
    _test.parseAssignments('otp_used = 1, reset_token = ?, department = NULL, attempts = attempts + 1, auth_version = auth_version + 1', ['token']),
    { set: { otp_used: 1, reset_token: 'token', department: null }, inc: { attempts: 1, auth_version: 1 }, paramIndex: 1 }
  );
